...
export function genDataChain(order: TradeInfo) {
    ...
    if (!MerchantID || !Version || !RespondType || !ReturnUrl) throw new Error('Missing required environment variables');
...
export function createSesEncrypt(TradeInfo: TradeInfo) {
    ...
    if (!HASHKEY || !HASHIV) throw new Error('Missing required environment variables');
